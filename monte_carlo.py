...
def is_game_over(board):
    ...
    if len(find_legal_moves(board)) == 0:  # Assuming find_legal_moves function is defined elsewhere
        return True
...
def get_game_result(board):
    ...
    for col in range(len(board[0])):
      for i in range(len(board) - 3):
        if board[i][col] == board[i + 1][col] == board[i + 2][col] == board[i + 3][col] and board[i][col] != 'O':
          return get_winner(board[i][col])

    # Check for diagonal wins.
    for i in range(len(board) - 3):
      for j in range(len(board[0]) - 3):
        if board[i][j] == board[i + 1][j + 1] == board[i + 2][j + 2] == board[i + 3][j + 3] and board[i][j] != 'O':
          return get_winner(board[i][j])

        if board[i][j + 3] == board[i + 1][j + 2] == board[i + 2][j + 1] == board[i + 3][j] and board[i][j + 3] != 'O':
          return get_winner(board[i][j + 3])
...
class MonteCarloTreeSearchNode():
  ...
  player: the player the Algo is trying to optomize
  ...
  """
  def __init__(self, state, parent=None, parent_action=None, player = 'Y',verbosity = 0):
    self.player = player
    self.state = state
    self.parent = parent
  ...
  def expand(self):
      ...
      while len(self._untried_actions) != 0:
        ...
        next_state = play_move(board , action)
        child_node = MonteCarloTreeSearchNode(next_state, parent=self, parent_action=action, player=self.player, verbosity=self.verbosity)
  ...
  def rollout(self):
      ...
      while not is_game_over(current_state):
         action = random.choice(find_legal_moves(current_state))
         current_state = play_move(current_state, action)
...
def main(file_path, verbosity, simulations):
    ...
    if algorithm == 'PMCGS':
      root = MonteCarloTreeSearchNode(state = board, player='Y', verbosity=v)
      selected_node = root.best_action(simulations = simulations)
      print('Selected Move: ' , selected_node.parent_action)
...
if __name__ == "__main__":
    file_path = sys.argv[1]  # The name of the input file
    verbosity = sys.argv[2]   # Verbose, Brief, or None
    simulations = int(sys.argv[3])  # Number of simulations, not used for UR but required for the script
